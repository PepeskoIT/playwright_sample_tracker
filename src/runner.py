...
async def runner(shop, engine, gui: bool, incognito: bool, channel, trace:bool):
    bot_client = SHOP_DATA[shop]["bot"]
    bot_accounts = SHOP_DATA[shop]["accounts"]

    async with async_playwright() as p:
        browser = await getattr(p, engine).launch(
            headless=not gui, channel=channel,
            args=[
        ...
        all_clients = (
            bot_client(browser=browser, account=account, incognito=incognito, trace=trace)
...
async def start():
    ...
    main_parser.add_argument(
        "-c", "--channel", type=str, required=False, help="channel", choices=["msedge", "msedge-dev"],
    )
    main_parser.add_argument(
    ...
    main_parser.add_argument(
        "--incognito", action='store_true', required=False, help="avoid storing session"
    )
    main_parser.add_argument(
        "--trace", action='store_true', required=False, help="record run trace"
    )
    main_parser.add_argument(
    ...
    while True:
        try:
            await runner(args.shop, args.engine, args.gui, args.incognito, args.channel, args.trace)
